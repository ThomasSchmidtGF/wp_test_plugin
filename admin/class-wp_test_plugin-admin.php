...
class Wp_test_plugin_Admin {
	...
	public function display_plugin_setup_page() {
		include_once( 'partials/wp_test_plugin-admin-display.php' );
	}
     ...
     * @since    1.0.0
     */
	private function wp_test_plugin_login_logo_css(){
		if(isset($this->wp_test_plugin_options['login_logo_id']) && !empty($this->wp_test_plugin_options['login_logo_id'])){
			$login_logo = wp_get_attachment_image_src($this->wp_test_plugin_options['login_logo_id'], 'thumbnail');
	...
	private function wp_test_plugin_login_background_color(){
		if(isset($this->wp_test_plugin_options['login_background_color']) && !empty($this->wp_test_plugin_options['login_background_color']) ){
			$background_color_css  = "body.login{ background:".$this->wp_test_plugin_options['login_background_color']."!important;}";
			return $background_color_css;
		}
	}
	// Get Button and links color is set and different from #00A0D2 return it's css
	private function wp_test_plugin_login_button_color(){
		if(isset($this->wp_test_plugin_options['login_button_primary_color']) && !empty($this->wp_test_plugin_options['login_button_primary_color']) ){
			$button_color = $this->wp_test_plugin_options['login_button_primary_color'];
	...
	public function wp_test_plugin_login_css(){
		if( !empty($this->wp_test_plugin_options['login_logo_id']) || $this->wp_test_plugin_login_background_color() != null || $this->wp_test_plugin_login_button_color() != null){
			echo '<style>';
			if( !empty($this->wp_test_plugin_options['login_logo_id'])){
				  echo $this->wp_test_plugin_login_logo_css();
			}
			if($this->wp_test_plugin_login_background_color() != null){
				  echo $this->wp_test_plugin_login_background_color();
			}
			if($this->wp_test_plugin_login_button_color() != null){
				  echo $this->wp_test_plugin_login_button_color();
			}
			echo '</style>';
		}
	}
